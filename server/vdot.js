...
/**
 * O2 cost of running at velocity v (meters/min)
 * Returns ml O2/kg/min
 */
...
/**
 * Calculate VDOT from a race/effort performance
 * @param {number} distanceMeters - distance in meters
 * @param {number} timeMinutes - time in minutes
 * @returns {number} VDOT value
 */
function calcVDOT(distanceMeters, timeMinutes) {
...
function predictRaceTime(vdot, distanceMeters) {
  // Bisection method: find t where calcVDOT(distance, t) = vdot
  let lo = 1;    // minimum 1 minute
  let hi = 600;  // maximum 10 hours

  for (let i = 0; i < 100; i++) {
    const mid = (lo + hi) / 2;
    const computed = calcVDOT(distanceMeters, mid);
    if (computed > vdot) {
      // Time too short (too fast) -> increase time
      lo = mid;
    } else {
      // Time too long (too slow) -> decrease time
      hi = mid;
    }
...
// Reference lookup tables for VDOT 30-85 (generated from Daniels-Gilbert formulas)
// 10K times in seconds, threshold pace (88% VO2max) in sec/km, interval pace (98% VO2max) in sec/km
const VDOT_TABLE = {
...
/**
 * Look up VDOT from threshold pace (sec/km) using the table
 * Interpolates between table entries
 */
function vdotFromThresholdPace(secPerKm) {
  const entries = Object.entries(VDOT_TABLE).map(([v, d]) => [Number(v), d.threshold]);
  // Threshold pace decreases as VDOT increases
...
/**
 * Look up VDOT from interval pace (sec/km) using the table
 * Interpolates between table entries
 */
function vdotFromIntervalPace(secPerKm) {
  const entries = Object.entries(VDOT_TABLE).map(([v, d]) => [Number(v), d.interval]);
  // Interval pace decreases as VDOT increases
  for (let i = 0; i < entries.length - 1; i++) {
    const [v1, p1] = entries[i];
    const [v2, p2] = entries[i + 1];
    if (secPerKm <= p1 && secPerKm >= p2) {
